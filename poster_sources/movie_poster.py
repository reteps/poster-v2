...
class ClassicultPoster(Poster):
    ...
    def create_poster(self):
        ...
        title_y = margin*2
        adjusted_title_y = title_y - additional_title_height
        for i, line in enumerate(title_wrap):
            draw.text((margin, adjusted_title_y + i * title_h), line, fill='black', anchor='lt', font=font)
        
        # img
        img_y = margin + title_y
        poster.paste(self.img, box=(margin, img_y))

        # title
        # line
        bar_height = 5 * self.size // 1000
        draw.rectangle((margin, margin, margin+self.size, margin+bar_height), fill='black')

        # year con't

        draw.text((margin+self.size, adjusted_title_y), self.year, anchor='rt',fill='black',font=year_font)

        right_side_x = (self.size + margin) // 2 + margin
        # colors
        color_y = img_y + margin + self.size
        ...
        # director
        d_text = f'Directed by {self.director}'
        director_font = self.font_percent(d_text, (self.size - margin) // 2, 'fonts/Gidole-Regular.ttf')
        _, director_h = draw.textsize(d_text, director_font)
        ...
        if len(self.writers) == 1:
            ...
            writer_font = self.font_percent(w_text, (self.size - margin) // 2, 'fonts/Gidole-Regular.ttf')
        ...
        # runtime / seasons
        bottom_font_size = 20 * self.size // 1000
        bottom_font = ImageFont.truetype('fonts/Gidole-Regular.ttf', bottom_font_size)
        ...
        time_w, _ = draw.textsize(self.runtime, bottom_font)
        release_w, release_h = draw.textsize(self.released, bottom_font)
        available_space = (self.size - margin) / 2 - time_w - release_w
        genre_x = available_space // 2 + time_w + margin
        draw.text((genre_x, self.height - margin), genres,anchor='ms',fill='black',font=bottom_font)

        # plot

        plot_font_size = 15 * self.size // 1000
        plot_font = ImageFont.truetype('fonts/Gidole-Regular.ttf', plot_font_size)

        max_plot_y = color_y + director_h + writer_h
        available_space = self.height - max_plot_y - margin - release_h
        lines, plot_line_height = self.wrap_text(self.plot, (self.size - margin) / 2, plot_font)
        plot_height = len(lines) * plot_line_height

        center_of_available_space = (available_space - plot_height) // 2
        self.justified_text_box((margin, max_plot_y + center_of_available_space), self.plot, (self.size - margin) / 2, bottom_font)
